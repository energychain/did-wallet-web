...
class Identity {
  constructor(config) {
    ...
    this.delegate = async function(to) {
      console.log({identifier:config.identity.publicKey,chainNameOrId:config.chainId,registry:config.registry,rpcUrl:config.rpcUrl,privateKey:config.identity.privateKey});
    ...
    this.changeOwner = async function(to) {
      ...
      console.log(ethrDid.changeOwner);
